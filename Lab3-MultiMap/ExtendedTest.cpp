...
void testAddIfNotPresent() {
    cout << "Test addIfNotPresent" << endl;

    MultiMap m1;
    MultiMap m2;

    // Populate m1 with some initial pairs
    m1.add(1, 1);
    m1.add(2, 2);
    m1.add(3, 3);

    // Populate m2 with additional pairs
    m2.add(2,44);
    m2.add(4, 44);
    m2.add(5, 55);

    // Call addIfNotPresent and check the return value
    int addedPairs = m1.addIfNotPresent(m2);
    assert(addedPairs == 2);  // Expect 2 pairs to be added

    // Check the contents of m1 after addIfNotPresent
    assert(m1.size() == 5);  // Total of 5 pairs in m1
    assert(m1.search(1).size() == 1);  // Existing pair
    assert(m1.search(2).size() == 1);  // Existing pair
    assert(m1.search(3).size() == 1);  // Existing pair
    assert(m1.search(4).size() == 1);  // New pair added
    assert(m1.search(5).size() == 1);  // New pair added

    // Check that m2 remains unchanged
    assert(m2.size() == 3);  // Total of 3 pairs in m2
    assert(m2.search(2).size() == 1);  // Existing pair
    assert(m2.search(4).size() == 1);  // Existing pair
    assert(m2.search(5).size() == 1);  // Existing pair
}
